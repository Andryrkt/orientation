    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex flex-wrap gap-1.5 mb-4">
        {enseignant.matieres.map((m) => (
          <span key={m} className="text-xs bg-slate-100 dark:bg-white/10 text-slate-600 dark:text-slate-300 rounded-full px-2 py-1">
            {m}
          </span>
        ))}
      </div>

      {enseignant.bio && <p className="text-slate-700 dark:text-slate-300 mb-4">{enseignant.bio}</p>}
      {enseignant.disponibilites && (
        <div className="mb-6">
          <h3 className="font-bold text-slate-800 dark:text-white mb-1">Disponibilités</h3>
          <p className="text-slate-600 dark:text-slate-400 text-sm">{enseignant.disponibilites}</p>
        </div>
      )}
      ...
      {user ? (
        rdvSent ? (
          <div className="mb-8 bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 text-sm rounded-lg px-4 py-3">
            Ta demande a été envoyée à {enseignant.prenom}. Tu recevras une réponse prochainement dans "Mes rendez-vous".
          </div>
        ) : (
