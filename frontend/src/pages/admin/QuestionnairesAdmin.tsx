  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Questionnaires d'orientation</h1>
        <button
      ...
      <p className="text-sm text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 rounded-md px-4 py-3 mb-6">
            ...
            {data?.map((q) => (
              <tr key={q.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40">
                <td className="px-4 py-3">
                  <Link to={`/admin/questionnaires/${q.id}`} className="text-brand-600 dark:text-brand-400 hover:underline font-medium">
                    {q.titre}
                  </Link>
                </td>
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{q._count?.questions ?? 0}</td>
                <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{q._count?.resultats ?? 0}</td>
                <td className="px-4 py-3">
                  <button
                    onClick={() => toggleActifMutation.mutate({ id: q.id, actif: !q.actif })}
                    className={`px-2 py-1 rounded-full text-xs font-medium ${
                      q.actif ? 'bg-green-100 text-green-700' : 'bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400'
                    }`}
                  >
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-20 p-4">
          <div className="bg-white dark:bg-slate-900 rounded-lg shadow-xl w-full max-w-lg">
            <form onSubmit={handleCreate} className="p-6 space-y-4">
              <h2 className="text-lg font-bold text-slate-800 dark:text-slate-100">Nouveau questionnaire</h2>
              <div>
                <label className="block text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">Titre <span className="text-red-500">*</span></label>
                <input
